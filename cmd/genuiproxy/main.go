...
// signature that forwards the call to the collector via the generic Invoke RPC.
//
// The generated file is declared `package app` (in directory internal/uiproxy,
// see proxy.go for why): Wails namespaces the JS bindings by the bound struct's
// Go package name, so the proxy must report as app.App to keep the frontend's
// window.go.app.App.* bindings working unchanged. Because this package and the
// domain package internal/app then share the name app, the domain import is
// aliased as appdom in the generated source.
//
...
const (
	...
	// appAlias is the local import name for the domain package internal/app in
	// the generated source. The generated file is itself `package app` (see the
	// command doc), so the domain import must be aliased to avoid two packages
	// named app in one file.
	appAlias = "appdom"
)
...
func assemble(imports map[string]string, methods []string) string {
	...
	for _, p := range paths {
		// The domain package internal/app shares the name app with this
		// generated package, so it is imported under the appAlias name.
		if p == appPkgPath {
			fmt.Fprintf(&b, "\t%s %q\n", imports[p], p)
		} else {
			fmt.Fprintf(&b, "\t%q\n", p)
